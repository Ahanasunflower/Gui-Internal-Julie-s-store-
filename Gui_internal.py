...
#Ask the customer if they want to quit
def quit():
    comfirmation = messagebox.askquestion("Quit","Are you are you want to quit?")
    if comfirmation == 'yes':
        window.quit()
...
def save_data():
    ...
    if firstname !="" and lastname !="":
        with open("customer_details1.txt", "a") as file:
            file.write(f"{firstname},{lastname},{item},{amount}\n")
...
def display_data():
    try:
        with open("customer_details1.txt", "r") as file:
            data = file.readlines()
...
def return_window():
    ...
    #spinbox is counter starts at certain number aned can be counted to another number
    global return_spinbox
    return_label = tkinter.Label(user_info_frame, text="Number returned", bg= "#f7dbc6")
    return_spinbox = tkinter.Spinbox(user_info_frame, from_=1, to=110)
    return_label.grid(row=2,column=1)
    return_spinbox.grid(row=3, column=1)
    ...
    button_save = tk.Button(frame, text="Save Return Details",command= save_data)
    button_save.grid(row=2,column=0, sticky="news", padx=20, pady=10)
...
#Return button
button2 = tk.Button(window, text = 'Return', command = return_window, width= 10, height= 2)
button2.pack(expand = True)

#Quit button
button3 = tk.Button(window, text = 'Quit', command = quit, width= 10, height= 2)
